  ...
  <login-form :formType="type">
    <template slot="form">
      <van-form validate-first>
        ...
        <span class="by-security-code" @click="byPsw = !byPsw">
          {{ byPsw ? '验证码登录' : '密码登录' }}
        </span>
        <van-button
          class="login-btn"
...
export default {
  ...
  methods: {
    ...
    login: async function() {
      ...
      const res = await this.$api.login(qs.stringify({ username, password }));
      const { status, info } = res.data;
      if (status === 1) {
        this.$notify({ type: 'success', message: info, duration: 1000 });
